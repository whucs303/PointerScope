...
MCObjectStreamer::MCObjectStreamer(MCContext &Context,
  ...
  /// xie: Synchronize storage structures in MCObjectStreamer to MCAssembler.
  Assembler->reorderInfo = &(this->reorderInfo);
  Assembler->FixupsData = &(this->FixupsData);
  Assembler->FixupsDataRel = &(this->FixupsDataRel);
  Assembler->FixupsInitArray = &(this->FixupsInitArray);
  Assembler->FixupsRodata = &(this->FixupsRodata);
  Assembler->FixupsText = &(this->FixupsText);
  Assembler->JumpTableTargets = &(this->JumpTableTargets);
...
// NOTE: Three branchs for real instruction
void MCObjectStreamer::EmitInstructionImpl(const MCInst &Inst,
  ...
  /// NOTE: 1. EmitInstToData() if doesn't need relaxation
  ...
  //   fragment.
  /// NOTE: 2. relaxInstruction() and EmitInstToData() if Bunding is enabled
  ...
  /// NOTE: 3. EmitInstToFragment() for other Instr. Note that we haven't monitored this type of Instr
