...
/// xie: Called by EmitInstToFragment() and EmitInstToData() when processing entity instructions.
void inline MCELFStreamer::checkBlockMark(const MCInst &Inst, MCFragment* frag, uint64_t offset){

  if (this->isFirstInstrInBBL){
    this->isFirstInstrInBBL = false;

    MCSectionELF* sec = static_cast<MCSectionELF*>(frag->getParent());
    if (sec->secLayout == nullptr) {
      sec->secLayout = static_cast<SecLayout *>(malloc(sizeof(SecLayout)));
      memset(sec->secLayout, 0, sizeof(SecLayout));
    }

    // Initialize a BasicBlockNode
    BasicBlockNode* bbl = static_cast<BasicBlockNode *>(malloc(sizeof(BasicBlockNode)));
    memset(bbl, 0, sizeof(BasicBlockNode));

    bbl->fragStart = frag;
    bbl->fragStartOffset = offset;

    bbl->type = BBL;
    if (this->isFirstInstrInFun){
      this->isFirstInstrInFun = false;
      if (sec->secLayout->bblListTail != nullptr)
        sec->secLayout->bblListTail->type = FUN;
    }
    bbl->fallThrough = this->fallThrough;

    // Append the BasicBlockNode to the sec->secLayout
    if (sec->secLayout->bblListHead == nullptr){
      sec->secLayout->bblListHead = bbl;
      sec->secLayout->bblListTail = bbl;
    }else{
      sec->secLayout->bblListTail->next = bbl;
      sec->secLayout->bblListTail = bbl;
    }
  }
}
...
void MCELFStreamer::EmitInstToFragment(const MCInst &Inst,
  ...
  /// xie: caller is MCObjectStreamer::EmitInstructionImpl() branch3
  checkBlockMark(Inst, &F, 0);
...
void MCELFStreamer::EmitInstToData(const MCInst &Inst,
  ...
  /// xie: caller is MCObjectStreamer::EmitInstructionImpl(), branch1 and branch2
  checkBlockMark(Inst, DF, DF->getContents().size());
