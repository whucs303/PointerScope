...
/// xie
std::tuple<unsigned, unsigned, unsigned, const MCSymbol*, const MCSymbol*, MCSection*, MCSection*>
MCAssembler::get_fixup_type(MCFixup Fixup,
                            MCValue Target,
                            MCAsmLayout &Layout,
                            std::string secName,
                            uint64_t offset,
                            std::unordered_map<std::string, std::unordered_map<uint64_t, unsigned>*> JmpStartAddr,
                            unsigned &currentJTLNum) {
  unsigned type = 0;

  bool is_pcrela = 0;
  const MCSymbol* symB = nullptr;      // base symbol
  unsigned base_type = 0;              // base type (OUT PC VALUE INDEX)
  unsigned base_bbl_sym = 0;           // value of base symbol
  MCSection* baseSection = nullptr;    // section name of base symbol

  const MCSymbol* symA = nullptr;      // target symbol
  unsigned target_type = 0;            // target type (OUT VALUE INDEX)
  unsigned target_bbl_sym = 0;         // value of target symbol
  MCSection* targetSection = nullptr;  // section name of target symbol

  bool isJTLFixup = false;

  if (Target.getSymA() != nullptr){
    symA = &(Target.getSymA()->getSymbol());

    if (!symA->isUndefined()){
      target_type = 2;
      target_bbl_sym = Layout.getSymbolOffset(*symA);
      targetSection = static_cast<MCSectionELF *>(&(symA->getSection()));
    }
  }

  if (Target.getSymB() != nullptr){
    symB = &(Target.getSymB()->getSymbol());

    if (!symB->isUndefined()){
      base_type = 2;
      base_bbl_sym =  Layout.getSymbolOffset(*symB);
      baseSection = static_cast<MCSectionELF*>(&(symB->getSection()));
    }
  }

  is_pcrela = getBackend().getFixupKindInfo(Fixup.getKind()).Flags &
                 MCFixupKindInfo::FKF_IsPCRel;
  if (symB)
    is_pcrela = true;
  unsigned reloc_type = getWriter().getRelocType(*this, Target, Fixup, is_pcrela);

  std::unordered_map<std::string, std::unordered_map<uint64_t, unsigned>*>::iterator iter1 = JmpStartAddr.find(secName);
  std::unordered_map<uint64_t, unsigned>::iterator iter2;
  if (iter1 != JmpStartAddr.end()){
    iter2 = iter1->second->find(Fixup.getOffset());
    if (iter2 != iter1->second->end()){
      currentJTLNum = iter2->second;
    }
  }
  if (currentJTLNum > 0){
    isJTLFixup = true;
  ...
  type |= target_type<<2;
  type |= (is_pcrela)<<4;
  type |= (isJTLFixup ? 1:0)<<6;
  type |= reloc_type << 16;

  return std::make_tuple(type, base_bbl_sym, target_bbl_sym, symA, symB, baseSection, targetSection);
}
...
void MCAssembler::layout(MCAsmLayout &Layout) {
  ...
  /// xie: 1. layout collect before
  const MCObjectFileInfo *MOFI = Layout.getAssembler().getContext().getObjectFileInfo();

  // tranver BasicBlock and cal bblSize
  for (MCSection &sec : *this) {
    MCSectionELF &ELFSec = static_cast<MCSectionELF &>(sec);
    std::string secName = ELFSec.getSectionName();
    bool firstBasicBlock = true;

    if (!(MOFI->getObjectFileType() == llvm::MCObjectFileInfo::IsELF && secName.find(".text")==0))
      continue;
    if (sec.secLayout == nullptr)
      continue;

    BasicBlockNode* bbl = sec.secLayout->bblListHead;
    while(bbl != nullptr){
      ...
      if (bbl->next == NULL){
        ...
        if (MCDataFragment* dataFragment = dyn_cast<MCDataFragment>(bbl->fragStart)){
          fragSize = dataFragment->getContents().size();
        }else if (MCCompactEncodedInstFragment* compactEncodedInstFragment = dyn_cast<MCCompactEncodedInstFragment>(bbl->fragStart)){
          fragSize = compactEncodedInstFragment->getContents().size();
        }else if (MCRelaxableFragment* relaxableFragment = dyn_cast<MCRelaxableFragment>(bbl->fragStart)){
          fragSize = relaxableFragment->getContents().size();
        }else if (MCDwarfLineAddrFragment* dwarfLineAddrFragment = dyn_cast<MCDwarfLineAddrFragment>(bbl->fragStart)){
          fragSize = dwarfLineAddrFragment->getContents().size();
        }else if (MCDwarfCallFrameFragment* dwarfCallFrameFragment = dyn_cast<MCDwarfCallFrameFragment>(bbl->fragStart)){
          fragSize = dwarfCallFrameFragment->getContents().size();
        }else if (MCCVDefRangeFragment* cvDefRangeFragment = dyn_cast<MCCVDefRangeFragment>(bbl->fragStart)){
          fragSize = cvDefRangeFragment->getContents().size();
        }else{
          exit(1);
        }
        end = bbl->fragStart->getOffset() + fragSize;
      }
      else {
        end = bbl->next->fragStart->getOffset() + bbl->next->fragStartOffset;
      }
      // determine the start
      if (firstBasicBlock){
        ...
        start = bbl->fragStart->getOffset() + bbl->fragStartOffset;
      }
      bbl->bblSize = end-start;
      bbl->section = &sec;
  ...
  std::unordered_map<std::string, std::unordered_map<uint64_t, unsigned>*> JmpStartAddr;
  std::unordered_map<std::string, std::unordered_map<uint64_t, unsigned>*>::iterator JmpStartAddrIter;
  std::unordered_map<uint64_t, unsigned>* SectionJmpMap; // need free
  std::unordered_map<uint64_t, unsigned>::iterator SectionJmpMapIter;
  unsigned currentJTLNum = 0;

  /// xie: 2. Traversing Jump Tables JTs
  std::map<std::string, unsigned>::iterator JTsIter;
  for (JTsIter = this->JumpTableTargets->begin(); JTsIter != this->JumpTableTargets->end(); JTsIter++) {
    // printf("%s\n", JTsIter->first.c_str());

    MCSymbol *JTISym = this->Context.lookupSymbol(JTsIter->first);
    if (JTISym) {
      if (!JTISym->isVariable()) {
        if (JTISym->getFragment()) {
          MCSectionELF *section = static_cast<MCSectionELF *>(JTISym->getFragment()->getParent());

          if (JmpStartAddr.find(section->getSectionName().str()) != JmpStartAddr.end()) {
            SectionJmpMap = JmpStartAddr[section->getSectionName().str()];
            (*SectionJmpMap)[JTISym->getFragment()->getOffset() + JTISym->getOffset()] = JTsIter->second;
          }
          else {
            SectionJmpMap = new std::unordered_map<uint64_t, unsigned>();
            (*SectionJmpMap)[JTISym->getFragment()->getOffset() + JTISym->getOffset()] = JTsIter->second;
            JmpStartAddr[section->getSectionName().str()] = SectionJmpMap;
          }
        }
      }
    }
  ...
  bool firstFixupInSec = true;
  MCValue Target;

  for (MCSection &Sec : Layout.getAssembler()) {
    MCSectionELF &ELFSec = static_cast<MCSectionELF &>(Sec);
    std::string secName = ELFSec.getSectionName();
    const char* secNameChar = secName.c_str();
    firstFixupInSec = true;

    for (MCFragment &Frag : Sec) {
      uint64_t fragOffset = Layout.getFragmentOffset(&Frag);
      ...
      if (auto *FragWithFixups = dyn_cast<MCDataFragment>(&Frag)) {
        ...
        continue;
      } else if (auto *FragWithFixups = dyn_cast<MCDwarfCallFrameFragment>(&Frag)) {
        Fixups = FragWithFixups->getFixups();
        Contents = FragWithFixups->getContents();
      } else
      ...
      for (const auto & Fixup : Fixups) {
        ...
        uint64_t FixedValue;
        bool IsResolved;
        std::tie(Target, FixedValue, IsResolved) = handleFixup(Layout, Frag, Fixup);
        getBackend().applyFixup(*this, Fixup, Target, Contents, FixedValue,
                                IsResolved, STI);

        /// xie: 3. layout collect before
        if (MOFI->getObjectFileType() == llvm::MCObjectFileInfo::IsELF) {

          unsigned type = 0;
          int64_t add = Target.getConstant();
          const MCSymbol* symB = nullptr;
          unsigned base_bbl_sym;
          MCSection* baseSection = nullptr;

          const MCSymbol* symA = nullptr;
          unsigned target_bbl_sym;
          MCSection* targetSection = nullptr;

          uint64_t offset = fragOffset + Fixup.getOffset();
          unsigned derefSize = getBackend().getFixupKindLog2Size(Fixup.getKind());

          std::tie(type, base_bbl_sym, target_bbl_sym,symA, symB, baseSection, targetSection) =
                  get_fixup_type(Fixup, Target, Layout, secName, offset, JmpStartAddr, currentJTLNum);

          if (!strncmp(secNameChar, ".text", 5)) {
            if (firstFixupInSec) {
              isNewTextSection = true;
              firstFixupInSec = false;
            } else
              isNewTextSection = false;

            type |= isNewTextSection<<5;
            this->FixupsText->push_back(std::make_tuple(offset, derefSize, type, base_bbl_sym, target_bbl_sym, &Sec, baseSection, targetSection, add, symA, symB));
          } else if (!strncmp(secNameChar, ".rodata", 7)) {
            if (firstFixupInSec) {
              isNewRodataSection = true;
              firstFixupInSec = false;
            } else
              isNewRodataSection = false;

            type |= isNewRodataSection<<5;
            this->FixupsRodata->push_back(std::make_tuple(offset, derefSize, type, base_bbl_sym, target_bbl_sym, &Sec, baseSection, targetSection, add, symA, symB));
          } else if (!strncmp(secNameChar, ".data.rel.ro", 12)) {
              if (firstFixupInSec) {
                isNewDataRelSection = true;
                firstFixupInSec = false;
              } else
                isNewDataRelSection = false;

              type |= isNewDataRelSection<<5;
              this->FixupsDataRel->push_back(std::make_tuple(offset, derefSize, type, base_bbl_sym, target_bbl_sym, &Sec, baseSection, targetSection, add, symA, symB));
          } else if (!strncmp(secNameChar, ".data", 5)) {
            if (firstFixupInSec) {
              isNewDataSection = true;
              firstFixupInSec = false;
            } else
              isNewDataSection = false;

            type |= isNewDataSection<<5;
            this->FixupsData->push_back(std::make_tuple(offset, derefSize, type, base_bbl_sym, target_bbl_sym, &Sec, baseSection, targetSection, add, symA, symB));

          } else if (!strcmp(secNameChar, ".init_array") || !strcmp(secNameChar, ".fini_array") || !strcmp(secNameChar, ".preinit_array")) {
            if (firstFixupInSec) {
              isNewInitSection = true;
              firstFixupInSec = false;
            } else
              isNewInitSection = false;

            type |= isNewInitSection<<5;
            this->FixupsInitArray->push_back(std::make_tuple(offset, derefSize, type, base_bbl_sym, target_bbl_sym, &Sec, baseSection, targetSection, add, symA, symB));
          }
        }
      }
    }
