...
public:
  ...
  /// xie:
  MCELFObjectTargetWriter* getMCELFObjectTargetWriter(){
    return TargetObjectWriter.get();
  }
...
unsigned ELFWriter::addToSectionTable(MCSectionELF *Sec) {
  ...
  /// xie: SectionTable is used by writeSectionHeader(), thus we use this order as secIdx
  Sec->secIdx = SectionTable.size();
...
void ELFWriter::writeSectionData(const MCAssembler &Asm, MCSection &Sec,
  ...
  /// xie: process the special section (.rand) to store the information for transformation
...
/// xie
void collectLayout(const MCAssembler &Asm, ShuffleInfo::ReorderInfo* reorderInfo){
  const MCObjectFileInfo *MOFI = Asm.getContext().getObjectFileInfo();

  for (MCSection &sec : Asm) {
    MCSectionELF &secELF = static_cast<MCSectionELF &>(sec);

    if (!(MOFI->getObjectFileType() == llvm::MCObjectFileInfo::IsELF && secELF.getSectionName().find(".text")==0))
      continue;
    if (sec.secLayout == nullptr)
      continue;

    /// Start collecting the layout metadata
    BasicBlockNode* bbl = sec.secLayout->bblListHead;
    while(bbl != nullptr) {

      ShuffleInfo::ReorderInfo_LayoutInfo* layoutInfo = reorderInfo->add_layout();
      layoutInfo->set_bb_size(bbl->bblSize);
      layoutInfo->set_bb_fallthrough(!(bbl->type == FUN || bbl->type == OBJ) &&bbl->fallThrough);
      layoutInfo->set_type(bbl->type);
      if (bbl->type == OBJ)
        layoutInfo->set_src_type(0);
      layoutInfo->set_section(bbl->section->secIdx);
      layoutInfo->set_info(static_cast<MCSectionELF*>(bbl->section)->getSectionName().str().c_str());
...
/// xie
void collectFixups(std::list<std::tuple<unsigned, unsigned, unsigned, unsigned, unsigned, MCSection*, MCSection*, MCSection*, int64_t, const MCSymbol*, const MCSymbol*>>* Fixups,
               ShuffleInfo::ReorderInfo_FixupInfo* fixupInfo, std::string secName)
{
  unsigned FixupOffset, FixupSize, type, base_sym_val, target_sym_val;
  MCSection* section;
  MCSection* baseSection;
  MCSection* targetSection;
  const MCSymbol* symA;
  const MCSymbol* symB;
  int64_t add;
  ...
  for (auto & Fixup : *Fixups)
  {
    ...
    std::tie(FixupOffset, FixupSize, type, base_sym_val, target_sym_val,
             section, baseSection, targetSection, add, symA, symB) = Fixup;

    pFixupTuple->set_add(add);
    pFixupTuple->set_offset(FixupOffset);
    pFixupTuple->set_section(section->secIdx);
    if (targetSection) {
      pFixupTuple->set_target_section(targetSection->secIdx);
    }
    if (baseSection) {
      pFixupTuple->set_base_section(baseSection->secIdx);
    }

    if ((type & 0x00000003) == 2 || (type & 0x00000003) == 3)
      pFixupTuple->set_base_bbl_sym(base_sym_val);
    if ((type & 0x0000000c) == 8 || (type & 0x0000000c) == 0xc)
      pFixupTuple->set_target_bbl_sym(target_sym_val);

    if (symA != NULL){
      if (symA->getIndex() != 0){
        type &= 0xFFFFFFF3; // Set the type of target to INDEX
        type |= 0x0000000c;
        //printf("symA: %s %d\n", symA->getName().str().c_str(), symA->getIndex());
        pFixupTuple->set_target_bbl_sym(symA->getIndex());
      }
    }
    if (symB != NULL){
      if (symB->getIndex() != 0){
        type &= 0xFFFFFFFC; // Set the type of base to INDEX
        type |= 0x00000003;
        //printf("symB: %s %d\n", symB->getName().str().c_str(), symB->getIndex());
        pFixupTuple->set_base_bbl_sym(symB->getIndex());
      }
    }

    pFixupTuple->set_type(type);
  }
}
...
uint64_t ELFWriter::writeObject(MCAssembler &Asm, const MCAsmLayout &Layout) {
  ...
  MCSectionELF* randSection = NULL; /// xie
  ...
  for (MCSection &Sec : Asm) {
    ...
    /// xie: we will write the rand section later
    if (Section.getSectionName().startswith(".rand"))
      randSection = &Section;
    else
  ...
  if (Mode == DwoOnly) {
    ...
    /// xie:=============Collect metadata and write to .rand section=============

    /// Step 1. Collect Metadata
    ShuffleInfo::ReorderInfo* reorderInfo = Layout.getAssembler().reorderInfo;

    ShuffleInfo::ReorderInfo_BinaryInfo* binaryInfo = reorderInfo->mutable_bin();
    binaryInfo->set_rand_obj_offset(0x0); // not used in gold static linker

    /// write layout metadata
    collectLayout(Asm, reorderInfo);

    /// write pointer metadata
    ShuffleInfo::ReorderInfo_FixupInfo* fixupInfo = reorderInfo->add_fixup();
    collectFixups(Layout.getAssembler().FixupsText, fixupInfo, ".text");
    collectFixups(Layout.getAssembler().FixupsRodata, fixupInfo, ".rodata");
    collectFixups(Layout.getAssembler().FixupsData, fixupInfo, ".data");
    collectFixups(Layout.getAssembler().FixupsDataRel, fixupInfo, ".data.rel.ro");
    collectFixups(Layout.getAssembler().FixupsInitArray, fixupInfo, ".init_array");

    /// Step 2.  Serialize metadata and write to the object file
    uint64_t SecStart = W.OS.tell();

    std::string randContents;
    if (!reorderInfo->SerializeToString(&randContents)) {
      errs() << "Failed to serialize the metadata to .rand section! \n";
    }
    W.OS << randContents;
    ...
    /// Step 3.
    SectionOffsets[randSection] = std::make_pair(SecStart, SecEnd);
...
/// xie
unsigned ELFObjectWriter::getRelocType(MCAssembler &Asm,
                                       const MCValue &Target,
                                       const MCFixup &Fixup,
                                       bool IsPCRel){
  MCContext &Ctx = Asm.getContext();
