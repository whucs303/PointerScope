...
/// xie:
unsigned WinCOFFObjectWriter::getRelocType(MCAssembler &Asm,
                                       const MCValue &Target,
                                       const MCFixup &Fixup,
                                       bool IsPCRel){
  return TargetObjectWriter->getRelocType(Asm.getContext(), Target, Fixup, Target.getSymB(), Asm.getBackend());
}
