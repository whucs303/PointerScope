...
void AsmPrinter::EmitFunctionBody() {
  ...
  /// xie: Collect JTI from MF in this last MachineFunctionPass(AsmPrinter)
  ...
  if (MJTI != nullptr) {
    ...
    if (!JT.empty()) {
      for (unsigned JTI = 0, e = JT.size(); JTI != e; ++JTI) {
        ...
        std::string MJTKey = ".LJTI" + std::to_string(MFID) + "_" + std::to_string(JTI);

        this->OutStreamer->JumpTableTargets[MJTKey] = JT[JTI].MBBs.size();
      }
    }
  }
  ...
  /// xie: record the function start
  this->OutStreamer->isFirstInstrInFun = true;
//  printf("%s\n", MF->getFunction().getName().str().c_str());

  for (auto &MBB : *MF) {
//    printf("InstNum: %d FallThrough:%d\n", MBB.size(), MBB.canFallThrough());
    // Print a label for the basic block.
    EmitBasicBlockStart(MBB);

    /// xie: record the basicblock start
    this->OutStreamer->isFirstInstrInBBL = true;
    this->OutStreamer->fallThrough = MBB.canFallThrough();
...
bool AsmPrinter::doFinalization(Module &M) {
  ...
  /// xie: Emit the .rand section in doFinalization()
