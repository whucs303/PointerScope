...
struct BasicBlockNode_{
  enum bblType type; // basic block type: basic block or function boundary.
  // 0 represents basic block. 1 represents function start. 2 represents object end
  bool fallThrough; // whether the basic block is fall through
  const char* sec; // which section the basic block belongs to
  MCFragment* fragStart; // this basic block belongs to which frag.
  uint64_t fragStartOffset;
  uint32_t bblSize;
  MCSection* section;

  struct BasicBlockNode_* next; // link next basic block
};
typedef struct BasicBlockNode_ BasicBlockNode;

struct SecLayout_{
  BasicBlockNode* bblListHead;
  BasicBlockNode* bblListTail;
};
typedef struct SecLayout_ SecLayout;
/// xie: ====================my global variables end========================
...
public:
  ...
  /// xie: Since AsmPrinter outputs MachineInstr via outStreamer, the metadata structure can be saved here

  /// 1. layout and fixup, defined in MCAsmInfo.h.
  ShuffleInfo::ReorderInfo reorderInfo;

  /// 2. Storing the pointer metadata of the current object file.
  mutable std::list<std::tuple<unsigned, unsigned, unsigned, unsigned, unsigned, MCSection*, MCSection*, MCSection*, int64_t, const MCSymbol*, const MCSymbol*>>
      FixupsText, FixupsRodata, FixupsData, FixupsDataRel, FixupsInitArray;

  /// 3. Storing the Jump Table
  mutable std::map<std::string, unsigned> JumpTableTargets;

  /// 4. Three global variables used in AsmPrinter
  bool isFirstInstrInBBL;
  bool isFirstInstrInFun;
  bool fallThrough;
