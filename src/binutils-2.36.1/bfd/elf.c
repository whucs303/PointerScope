...
/// xie: 当gas最后给每个节去write_contents时，调用这个函数来为符号分配index。 注意：local符号在前
static bfd_boolean
  ...
  for (asect = abfd->sections; asect; asect = asect->next)
    {
      ...
      if (!ignore_section_sym (abfd, sym) && sect_syms[asect->index] == NULL)
	{
...
//    printf("name=%s offset=%lx\n", i_shdrp->bfd_section->name, offset);
					 ...
					 struct bfd_link_info *link_info)
{
  ...
  /// 为另外的两个节symtab和strtab和shstrtab，创建hdr
  ...
  /// 第一遍遍历为非rel的节设置offset，相当于是在二进制文件中先写这些能够确定的节(shstrtab呢？不在链上？)
...
_bfd_elf_write_object_contents (bfd *abfd)
{
  ...
  /// 为不load的节设置offset(为什么不在_bfd_elf_set_object_contents()中设置？)
