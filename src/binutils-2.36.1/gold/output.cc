...
Output_data_got<got_size, big_endian>::Got_entry::write(
    ...
    unsigned char* pov, Layout* my_layout, uint64_t got_offset) const
{
    /// xie: Whether to record this Got Entry
  ...
  /// xie: record this GOT Entry
  if (my_layout && need_record) {
      ShuffleInfo::ReorderInfo_FixupInfo *fixupInfo = my_layout->get_global_shuffleInfo().mutable_fixup(0);
      ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple *fixupTuple = fixupInfo->add_got();

      /// determine the r_type
      ...
      if (got_size == 64 ){
          ...
          printf("Unable to determine the r_type of this got pointer (got_size=%d)\n", got_size);
          exit(0x1);
      }
      ...
      type |= 1 << 7;                  // isRAND=True
      type |= 0 << 8;                  // isRELOC=False
      type |= r_type << 16;            // r_type
      fixupTuple->set_type(type);

      fixupTuple->set_offset(got_offset);
      fixupTuple->set_target_bbl_sym(val);
  }
...
Output_data_got<got_size, big_endian>::do_write(Output_file* of)
{
  ...
  /// xie: Get layout from output_section
  ...
  for (unsigned int i = 0; i < this->entries_.size(); ++i)
    {
      this->entries_[i].write(i, pov, my_layout, got_offset); /// xie: add this GOT Entry
      got_offset += add;
      pov += add;
    }
...
Output_section::Output_section(const char* name, elfcpp::Elf_Word type,
			       elfcpp::Elf_Xword flags, Layout* my_layout) /// xie
  : name_(name),
    addralign_(0),
    entsize_(0),
    load_address_(0),
