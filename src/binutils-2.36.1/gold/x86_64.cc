  ...
  class Relocate
  {
   ...
   private:
    ...
    relocate_tls(const Relocate_info<size, false>*, Target_x86_64*,
		 ...
		 unsigned char*, typename elfcpp::Elf_types<size>::Elf_Addr,
		 section_size_type,
         ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple* fixup_tuple); /// xie
...
//    }
  ...
  /// xie: TLS optimization is necessary for static links && dynamic to static.
  /// In this case we need to allow TLS optimization and disable metadata merging
  if (parameters->doing_static_link() && dynamic2exec){
      if (layout != NULL)
          layout->need_close_rand_output = true;
  }
  /// xie: TLS optimization may lead to inaccurate metadata collection.
  /// The temporary solution is to disable TLS optimization and then gradually adapt this mechanism in the future
  else
      return tls::TLSOPT_NONE;
  return op_retu;
}
    ...
    section_size_type view_size)
{
  ...
  // Pick the value to use for symbols defined in the PLT.
    /// xie: Traverse each relocation item and label the corresponding fixup.
    ...
    if (need_output_rand && relinfo->object->has_rand){
        ...
        if (fixup_tuple != nullptr){
            // Set the 9th bit to 1 to indicate that the fixup is a relocation item
            fixup_tuple->set_type( fixup_tuple->type() | 256);
        }
    }

    bool update_psymval = false;
    uint64_t new_target_value = 0;
    Symbol_value<size> symval;
    /// xie: Target address changed due to plt symbol adjustment.
    ...
    /// xie: Target address changed due to ifunc symbol adjustment.
    ...
    /// xie: Target address changed due to SHF_MERGE
    else if (!psymval->has_output_value()) {
        target_address = psymval->value(object, rela.get_r_addend());
        update_psymval = true;
    }

    /// xie: Depending on whether the target_sym has changed or not, we need the corresponding pointer metadata
    ...
    /// xie: If the relocation is not recorded in the pointer metadata, we need to create a new entry
    need_add = need_output_rand && my_layout->is_fixup_section(os->name()) && !relinfo->object->has_rand;
  ...
  switch (r_type)
	...
	Reloc_funcs::pcrela64(view, object, psymval, addend, got_address); /// xie
      }
      break;
      ...
      if (rela.get_r_addend() == -4)
	{
	  ...
	  if (!parameters->incremental()
	    ...
	    {
	      view[-2] = 0x8d;
	      new_reloc = elfcpp::R_X86_64_PC32;
	      Reloc_funcs::pcrela32(view, object, psymval, addend, address);
	      converted_p = true;

	      /// xie: Update fixup accordingly after the GOT Optimization
          if (need_output_rand && fixup_tuple != nullptr){
    ...
    case elfcpp::R_X86_64_TPOFF32:          // Local-exec
      ...
      this->relocate_tls(relinfo, target, relnum, rela, r_type, gsym, psymval,
			 view, address, view_size, fixup_tuple); /// xie
      break;
    ...
    if (need_output_rand && need_update && fixup_tuple != nullptr){
        if (has_record){
            ...
            printf("Cannot find the pointer\n");
            exit(1);
        }
    }

    /// xie: Add new pointer metadata entries
    if (need_output_rand && need_add) {
        if (has_record){
            my_layout->yarp_add_fixups(new_address, os->name(), target_address, base_address, target_add, new_reloc);
        }else{
            printf("Cannot find the pointer\n");
            exit(1);
        }
...
Target_x86_64<size>::Relocate::relocate_tls(
    ...
    ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple* fixup_tuple) /// xie
{
//    auto *my_layout = const_cast<Layout *>(relinfo->layout); /// xie
