			    ...
			    Output_section_order order, bool is_relro)
{
  ...
  if ((flags & elfcpp::SHF_ALLOC) == 0
      ...
      if (type == elfcpp::SHT_PROGBITS)
	{
      ...
      os = target->make_output_section(name, type, flags, this); /// xie
    }
...
/// xie
bool is_special_section_by_name(const char* secname){
...
/// xie
bool Layout::is_fixup_section(const char* sec_name) {
    if(!strncmp(sec_name, ".data.rel.ro", 12) ||
       !strcmp(sec_name, ".init_array") || !strcmp(sec_name, ".fini_array") || !strcmp(sec_name, ".preinit_array") ||
       !strncmp(sec_name, ".text", 5) ||
       !strncmp(sec_name, ".data", 5) ||
       !strncmp(sec_name, ".rodata", 7) ||
...
/// xie: Add a new fixup entry in pointer metadata
void Layout::yarp_add_fixups(uint64_t fixup_offset, const char *sec_name,
              uint64_t target_addr, uint64_t base_addr, int64_t add, unsigned int r_type){

    ShuffleInfo::ReorderInfo_FixupInfo *fixupInfo = this->get_global_shuffleInfo().mutable_fixup(0);
    ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple *fixupTuple = NULL;

    std::unordered_map<uint64_t, ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple*> fixup_map;
	if(!strncmp(sec_name, ".data.rel.ro", 12)){
        fixupTuple = fixupInfo->add_datarel();
	}
	else if(!strcmp(sec_name, ".init_array") || !strcmp(sec_name, ".fini_array") || !strcmp(sec_name, ".preinit_array")) {
        fixupTuple = fixupInfo->add_initarray();
    }
	else if (!strcmp(sec_name, ".text")) {
        fixupTuple = fixupInfo->add_text();
    }
	else if(!strncmp(sec_name, ".data", 5)){
        fixupTuple = fixupInfo->add_data();
	}
	else if(!strcmp(sec_name, ".rodata")) {
        fixupTuple = fixupInfo->add_rodata();
    }
    else if (!strcmp(sec_name, ".init")){
        fixupTuple = fixupInfo->add_init();
    }
    else if (!strcmp(sec_name, ".fini")){
        fixupTuple = fixupInfo->add_fini();
    }
    else{
        printf("Cannot add fixup to %s\n", sec_name);
        exit(1);
    }

    fixupTuple->set_info(sec_name);
    u_int32_t type = 0;
    type |= 0;                       // baseType=NONE
    type |= 2 << 2;                  // targetType=VALUE
    type |= 0 << 6;                  // isJump=False
    type |= fixup_map.count(fixup_offset)>0 ? 1:0 << 7;
    type |= 1 << 8;                  // isRELOC=TRUE
    type |= r_type << 16;
    fixupTuple->set_type(type);

    fixupTuple->set_offset(fixup_offset);
    fixupTuple->set_target_bbl_sym(target_addr);
    fixupTuple->set_base_bbl_sym(base_addr);
    fixupTuple->set_add(add);
}
...
/// xie: Determine if this output contains metadata
template<int size, bool big_endian>
void Layout::is_need_output_rand() {

    // If static && dynamic2exec, then enable TLS optimization and disable metadata merging
    if(this->need_close_rand_output) {
        this->need_output_rand = false;
    }
    // If --relocatable, then check whether input files contains .rand sections, we simply merge them.
    else if(parameters->options().relocatable()){
        if (this->object_has_rand) {
            this->need_output_rand = true;
        }
    }
    // If not --relocatable，then check whether whether there are non-empty text sections containing .rand, we will merge them
    else{
        for (Section_list::const_iterator p = this->section_list_.begin(); p != this->section_list_.end(); ++p) {
            ...
            if (!strcmp(os->name(), ".text")) {
                ...
                for (auto q = input_section_list.begin(); q != input_section_list.end(); q++) {
                    ...
                    if (!is_special_section_by_name(q->insname_->c_str()) &&
                        q_obj->has_rand &&
                        q->data_size() != 0) {
    ...
    if (!this->need_output_rand) {
        gold_info("Not merge sub-metadata");
    }
}

/// xie: Merge sub-metadata
template<int size, bool big_endian>
void
Layout::update_shuffleInfo(Symbol_table *symtab) {

        for (Section_list::const_iterator p = this->section_list_.begin(); p != this->section_list_.end();
             ++p) {
            Output_section *os = *p;
            Output_section::Input_section_list input_section_list = os->input_sections();
            ...
            const unsigned char *rand_contents;
            ShuffleInfo::ReorderInfo reorder_info;
            section_size_type rand_sec_len;

            if (!strcmp(os->name(), ".text")) {

                // Determining the code boundaries for randomization
                Output_section::Input_section_list::const_iterator input_section_randed_start = input_section_list.end();
                Output_section::Input_section_list::const_iterator input_section_randed_end = input_section_list.end();
                for (auto q = input_section_list.begin(); q != input_section_list.end(); q++) {
                    if (!(q->is_input_section() && q->relobj()))
                        continue;
                    q_obj = reinterpret_cast<Sized_relobj_file<size, big_endian> *>(q->relobj());
                    ...
                    if (!is_special_section_by_name(q->insname_->c_str()) &&
                            q_obj->has_rand &&
                            q->data_size() != 0 &&
                            input_section_randed_start == input_section_list.end()) {
                        input_section_randed_start = q;
                    }

                    if (input_section_randed_start != input_section_list.end() && q_obj->has_rand)
                        input_section_randed_end = q;
                }

                bool in_rand_range = false;
                for (auto q = input_section_list.begin(); q != input_section_list.end(); q++) {
                    if (!(q->is_input_section() && q->relobj()))
                        continue;
                    q_obj = reinterpret_cast<Sized_relobj_file<size, big_endian> *>(q->relobj());
                    ...
                    if (q >= input_section_randed_start && q <= input_section_randed_end)
                        in_rand_range = true;
                    else{
                        in_rand_range = false;
                    }
                    ...
                    if (q_obj->has_rand) {
                        rand_contents = q_obj->section_contents(q_obj->rand_ndx, &rand_sec_len, false);
                        std::string rand_input(reinterpret_cast<const char *>(rand_contents), rand_sec_len);
                        reorder_info.ParseFromString(rand_input);
                    }

                    // Merge pointer metadata. (Note that each obj is collected only once)
                    if (!q_obj->is_fixup_has_added() && q_obj->has_rand) {
                        merge_fixup(reorder_info, q_obj, symtab);
                        q_obj->set_fixup_has_added();
                    }

                    // We only need to merge the layout metadata in the randomized range
                    if (in_rand_range) {

                        // Calculate the input section size
                        uint64_t insize = 0;

                        q_next = q + 1;
                        while (q_next != input_section_list.end() && !(q_next->is_input_section() && q_next->relobj() != NULL && q_next->data_size() != 0))
                            q_next++;

                        if(q_next == input_section_list.end()){
                            insize = q->data_size();
                        }else{
                            q_next_obj = reinterpret_cast<Sized_relobj_file<size, big_endian> *>(q_next->relobj());
                            off_next = q_next_obj->section_offsets()[q_next->shndx()];
                            off = q_obj->section_offsets()[q->shndx()];
                            insize = off_next - off;
                        }

                        if (q_obj->has_rand) {
                            if (q->data_size() > 0){
                                merge_layout(reorder_info, q_obj, q->insname_->c_str(), q->shndx(), insize);
                            }
                        }
                        // We need to create a temporary layout entry if the object file is missing metadata.
                        else {
                            if (q->data_size() > 0) {
                                ShuffleInfo::ReorderInfo &gsi = get_global_shuffleInfo();

                                ShuffleInfo::ReorderInfo_LayoutInfo *layout = gsi.add_layout();
                                layout->set_bb_size(insize);
                                layout->set_type(3); // BBL_TYPE   = {0: "BBL", 1: "FUN", 2: "OBJ", 3: "OBJ_TEMP"}
                                layout->set_bb_fallthrough(false);
                                layout->set_info(str_concat(q->insname_->c_str(), q_obj->name().c_str()));
                                layout->set_src_type(2);
                            }
                        }
                    }
                }
//                gold_info("%ld/%ld", this->hybrid_obj, this->all_obj);
            }
        }
    }
...
/// Merge layout sub-metadata
template<int size, bool big_endian>
void
Layout::merge_layout(ShuffleInfo::ReorderInfo &to_add_info,
                     Sized_relobj_file<size, big_endian> *pfrom_object,
                     const char *input_section_name,
                     uint32_t input_section_index,
                     uint64_t input_section_size) {
        ShuffleInfo::ReorderInfo &gsi = get_global_shuffleInfo();

        ShuffleInfo::ReorderInfo_BinaryInfo *bin = gsi.mutable_bin();
        if (shuffle_info_.first_reorder_info) {
            shuffle_info_.first_reorder_info = false;
            bin->set_rand_obj_offset(pfrom_object->section_offsets()[input_section_index]);
        }

        uint64_t bbl_size = 0;

        ShuffleInfo::ReorderInfo_LayoutInfo *playoutinfo;
        for (int i=0; i < to_add_info.layout_size(); ++i) {
            const ShuffleInfo::ReorderInfo_LayoutInfo& bbl = to_add_info.layout(i);

            if (input_section_index != bbl.section())
                continue;

            playoutinfo = gsi.add_layout();
            playoutinfo->set_type(bbl.type());
            playoutinfo->set_bb_fallthrough(bbl.bb_fallthrough());
//            playoutinfo->set_info(str_concat(input_section_name, pfrom_object->name().c_str()));
            playoutinfo->set_bb_size(bbl.bb_size());

            bbl_size += bbl.bb_size();

            if (bbl.type() == OBJ_BBL) {
                playoutinfo->set_bb_size(bbl.bb_size() + (input_section_size-bbl_size));
                playoutinfo->set_src_type(bbl.src_type());
                break;
            }
        }
    }
...
/// Merge pointer sub-metadata
template<int size, bool big_endian>
void
Layout::merge_fixup(ShuffleInfo::ReorderInfo &to_add_info,
                    Sized_relobj_file<size, big_endian> *pfrom_object,
                    Symbol_table *symtab) {
    ShuffleInfo::ReorderInfo &gsi = get_global_shuffleInfo();
    ShuffleInfo::ReorderInfo_FixupInfo *pfixupinfo = NULL;
    ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple *fixup_output = NULL;

    if (!gsi.fixup_size())
        pfixupinfo = gsi.add_fixup();
    else
        pfixupinfo = gsi.mutable_fixup(0);

    for (int i = 0; i < to_add_info.fixup_size(); ++i) {
        const ShuffleInfo::ReorderInfo_FixupInfo& fixup_info = to_add_info.fixup(i);

        for (int j_1 = 0; j_1 < fixup_info.text_size(); ++j_1) {
            const ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple& fixup_input = fixup_info.text(j_1);

            if (!pfrom_object->section_exist(fixup_input.section()))
                continue;

            fixup_output = pfixupinfo->add_text();
            set_fixup(fixup_output, fixup_input, pfrom_object, symtab, 1);
        }

        for (int j_2 = 0; j_2 < fixup_info.rodata_size(); ++j_2) {
            const ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple& fixup_input = fixup_info.rodata(j_2);

            if (!pfrom_object->section_exist(fixup_input.section()))
                continue;

            fixup_output = pfixupinfo->add_rodata();
            set_fixup(fixup_output, fixup_input, pfrom_object, symtab, 2);
        }

        for (int j_3 = 0; j_3 < fixup_info.data_size(); ++j_3) {
            const ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple& fixup_input = fixup_info.data(j_3);

            if (!pfrom_object->section_exist(fixup_input.section()))
                continue;

            fixup_output = pfixupinfo->add_data();
            set_fixup(fixup_output, fixup_input, pfrom_object, symtab, 3);
        }

        for (int j_4 = 0; j_4 < fixup_info.datarel_size(); ++j_4) {
            const ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple& fixup_input = fixup_info.datarel(j_4);

            if (!pfrom_object->section_exist(fixup_input.section()))
                continue;

            fixup_output = pfixupinfo->add_datarel();
            set_fixup(fixup_output, fixup_input, pfrom_object, symtab, 4);
        }

        for (int j_5 = 0; j_5 < fixup_info.initarray_size(); ++j_5) {
            const ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple& fixup_input = fixup_info.initarray(j_5);

            if (!pfrom_object->section_exist(fixup_input.section()))
                continue;

            fixup_output = pfixupinfo->add_initarray();
            set_fixup(fixup_output, fixup_input, pfrom_object, symtab, 5);
        }
    }
}

/// xie: Repair each entry after merging pointer sub-metadata
template<int size, bool big_endian>
void Layout::set_fixup(ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple *fixup_output,
                      const ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple &fixup_input,
                      Sized_relobj_file<size, big_endian> *pfrom_object, Symbol_table *symtab, int sec_flag) {

    typedef typename elfcpp::Elf_types<size>::Elf_Addr Address;
    const std::vector<Address> &out_offsets(pfrom_object->section_offsets());
    Output_section* output_sections = pfrom_object->get_output_section(fixup_input.section());
    uint64_t fixup_offset = fixup_input.offset() + out_offsets[fixup_input.section()];

    output_sections->fixup_map[fixup_offset] = fixup_output;
    fixup_output->set_info(pfrom_object->get_output_section(fixup_input.section())->name());
    //fixup_output->set_info(str_concat(pfrom_object->get_output_section(fixup_input.section())->name(), pfrom_object->name().c_str()));
    //fixup_output->set_info(pfrom_object->get_output_section(fixup_input.section())->name());

    fixup_output->set_offset(fixup_offset);
    fixup_output->set_add(fixup_input.add());
    if (fixup_input.has_step())
        fixup_output->set_step(fixup_input.step());
    fixup_output->set_type( fixup_input.type() | 128);


    unsigned base_type = fixup_input.type() & 3;
    // When storing INDEX, we need to convert the symbol to the corresponding address
    if (base_type == 3) {
        uint64_t base_addr = get_symbol_value(pfrom_object, symtab, fixup_input.base_bbl_sym());
        fixup_output->set_base_bbl_sym(base_addr);
    }
    // When storing VALUE, we need to add it to the section address where it is located
    else if (base_type == 2) {
        fixup_output->set_base_bbl_sym(
                cal_symbol_addr(pfrom_object, fixup_input.base_bbl_sym(), fixup_input.base_section()));
    }
    // Finally we change the base_type to VALUE
    if (base_type == 3 || base_type == 2)
        fixup_output->set_type( (fixup_output->type() & 0xFFFFFFFC) | 0x00000002 );

    unsigned target_type = (fixup_input.type() >> 2) & 3;
    if (target_type == 3) {
        uint64_t target_addr = get_symbol_value(pfrom_object, symtab,
                                                fixup_input.target_bbl_sym());
        fixup_output->set_target_bbl_sym(target_addr);
    } else if (target_type == 2){
        fixup_output->set_target_bbl_sym(
                cal_symbol_addr(pfrom_object, fixup_input.target_bbl_sym(), fixup_input.target_section()));
    }
    if (target_type == 3 || target_type == 2)
        fixup_output->set_type( (fixup_output->type() & 0xFFFFFFF3) | 0x00000008);
}

template<int size, bool big_endian>
inline uint64_t cal_symbol_addr(Sized_relobj_file<size, big_endian> *pfrom_object, uint64_t offset, uint32_t sec_index) {
    if (pfrom_object->section_exist(sec_index)){
        // 1. Offset of input_section in output_section
        uint64_t input_section_offset = pfrom_object->get_output_section_offset(sec_index);
        // 2. Base address of the output_section
		 ...
		 Target* target, const Task* task)
{
  ...
  if (ehdr_start != NULL && ehdr_start->is_predefined())
    ...
    /// xie: Determine if need to perform sub-metadata merging
    this->is_need_output_rand<64, false>();

    /// xie: Insert .rand section in the final binary
    if (this->need_output_rand) {
        this->rand = this->choose_output_section(NULL, ".rand", elfcpp::SHT_PROGBITS, 0, false, ORDER_INVALID, false, false, false);
        this->rand->set_after_input_sections(); // Set to write the .rand section after the section table
        this->any_postprocessing_sections_ = true; //Notify gold that we also need to finalize at Write_after_input_sections_task
    }
  ...
  /// xie: We have already finalized the layout, now we can merge the sub-metadata
  if (this->need_output_rand && !parameters->options().relocatable())
    this->update_shuffleInfo<64, false>(symtab);
...
{
    /// xie: Serialize the message and write it to the .rand section
    Output_section_data* odata;
    if (this->need_output_rand) {
        const std::string metadata = this->get_global_shuffleInfo().SerializeAsString();
        odata = new Output_data_const(metadata.c_str(), metadata.length(), 1);
