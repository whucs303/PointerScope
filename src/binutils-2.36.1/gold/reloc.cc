						 ...
						 Output_file* of)
{
  ...
  this->write_sections(layout, pshdrs, of, &views, symtab); /// xie
...
/// xie
template<int size, bool big_endian>
void Sized_relobj_file<size, big_endian>::set_fixup(ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple *fixup_output,
                           const ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple &fixup_input,
                           const Symbol_table *symtab,
                           const std::vector<Output_section*>& out_sections,
                           const std::vector<Address>& out_offsets) {

    uint32_t sec_idx = fixup_input.section();

    fixup_output->set_section(out_sections[sec_idx]->do_out_shndx());
    fixup_output->set_offset(out_offsets[sec_idx] + fixup_input.offset());
    fixup_output->set_add(fixup_input.add());
    if (fixup_input.has_step())
        fixup_output->set_step(fixup_input.step());
    fixup_output->set_type(fixup_input.type());

    unsigned base_type = fixup_input.type() & 3;
    // In the case of storing INDEX, the index of the symbol needs to be updated
    if (base_type == 3) {
        uint64_t base_symidx = fixup_input.base_bbl_sym();
        uint64_t new_base_symidx;
        if (base_symidx < this->local_symbol_count_){
        ...
        if (new_base_symidx == -1U)
            gold_error("new_base_symidx=-1");
        fixup_output->set_base_bbl_sym(new_base_symidx);
    }
    // In the case of storing VALUE, we need to update the base_bbl_sym
    else if (base_type == 2) {
        uint32_t base_section_index = fixup_input.base_section();
        fixup_output->set_base_section(out_sections[base_section_index]->do_out_shndx());
        fixup_output->set_base_bbl_sym(out_offsets[base_section_index] + fixup_input.base_bbl_sym());
    }

    unsigned target_type = (fixup_input.type() >> 2) & 3;
    if (target_type == 3) {
        uint64_t target_symidx = fixup_input.target_bbl_sym();
        uint64_t new_target_symidx;
        if (target_symidx < this->local_symbol_count_){
        ...
        if (new_target_symidx == -1U)
            gold_error("new_target_symidx=-1");
        fixup_output->set_target_bbl_sym(new_target_symidx);
    } else if (target_type == 2){
        uint32_t target_section_index = fixup_input.target_section();
        fixup_output->set_target_section(out_sections[target_section_index]->do_out_shndx());
        fixup_output->set_target_bbl_sym(out_offsets[target_section_index] + fixup_input.target_bbl_sym());
    }
}
                            ...
                            const Symbol_table* symtab) /// xie
{
  ...
  bool is_sorted = true;
    ...
    /// xie: If --relocatable, we merge pointer metadata in this function.
    ShuffleInfo::ReorderInfo partial_shuffle_info;
    Layout* my_layout = const_cast<Layout*>(layout);
    ShuffleInfo::ReorderInfo &global_shuffle_info = my_layout->get_global_shuffleInfo();
    ...
    if (my_layout->need_output_rand && parameters->options().relocatable() && this->has_rand){
        ...
        partial_shuffle_info.ParseFromString(rand_input);
    }
    ...
    if (my_layout->need_output_rand && parameters->options().relocatable() && this->has_rand) {
        ...
        if (!global_shuffle_info.fixup_size())
            pfixupinfo = global_shuffle_info.add_fixup(); //if we don't have one such global field yet
        else
            pfixupinfo = global_shuffle_info.mutable_fixup(0);//if we already have it

        for (int i = 0; i < partial_shuffle_info.fixup_size(); ++i) {
            gold_assert(i < 1 && "[Error] Object files containing multiple FixupInfo");
            const ShuffleInfo::ReorderInfo_FixupInfo& fixup_info = partial_shuffle_info.fixup(i);
            ...
            for (int j_1 = 0; j_1 < fixup_info.text_size(); ++j_1) {
                ...
                if (!this->section_exist(fixup_input.section()))
                    continue;
                fixup_output = pfixupinfo->add_text();
                set_fixup(fixup_output, fixup_input, symtab, out_sections, out_offsets);
            }
  ...
  for (unsigned int i = 1; i < shnum; ++i, p += This::shdr_size)
	...
	continue;
      ...
      /// xie: If --relocatable, we merge layout metadata in this function.
      if (my_layout->need_output_rand && !strcmp(".text", os->name()) && parameters->options().relocatable()){

          std::string is_name = this->section_name(i);
          Address text_offset = out_offsets[i];

          //gold_info("ObjectFile:%s Section:%s TextOffset=%lx curSize=%lx viewSize=%lx", this->name().c_str(), is_name.c_str(), text_offset, my_layout->cur_size, view_size);

          if(my_layout->last_layout == NULL && text_offset != 0) {
              gold_error("ObjectFile:%s Section:%s TextOffset=%lx The first input_section of the .text section has a non-zero offset", this->name().c_str(),
                         is_name.c_str(), text_offset);
              exit(1);
          }

          // Padding of the previous bbl according to the current offset
          uint64_t padding = text_offset - my_layout->cur_size;
          my_layout->cur_size = text_offset;
          if (padding != 0 && my_layout->last_layout != NULL){
              my_layout->last_layout->set_bb_size(my_layout->last_layout->bb_size() + padding);
          }

          /// 录入Layout的时候分以下三种情况
          /// case1. 特殊的.text节，此时作为obj_temp
          /// case2. 为.text但没有rand节
          ShuffleInfo::ReorderInfo_LayoutInfo* playoutinfo;
          if (is_name != ".text" || (is_name == ".text" and !this->has_rand)) {
              playoutinfo = global_shuffle_info.add_layout();
              playoutinfo->set_section(os->do_out_shndx());
              playoutinfo->set_type(3);
              playoutinfo->set_bb_fallthrough(false);
              playoutinfo->set_info(str_concat(is_name.c_str(), this->name().c_str()));
              playoutinfo->set_bb_size(view_size);
              playoutinfo->set_src_type(2);
              my_layout->cur_size += view_size;
              my_layout->last_layout = playoutinfo;
          }
          /// case3. 为.text且有rand节
          else{
              // 合并子元数据中的layout
              for (int j=0; j < partial_shuffle_info.layout_size(); ++j) {
                  const ShuffleInfo::ReorderInfo_LayoutInfo& bbl = partial_shuffle_info.layout(j);

                  /// xie: 只layout中挑选出该input_section名字的BBL以加入
                  if (i != bbl.section())
                      continue;
                  ...
                  my_layout->cur_size += bbl.bb_size();
                  my_layout->last_layout = playoutinfo;
              }
          }
      }
