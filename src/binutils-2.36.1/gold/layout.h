...
class Layout
{
 public:
    ...
    /// xie: Store the metadata in layout
    struct Shuffle_Info{
        ShuffleInfo::ReorderInfo global_reorder_info;
        bool first_reorder_info = true;
    } shuffle_info_;

    bool object_has_rand = false;
    bool need_close_rand_output = false;
    bool need_output_rand = false;
    Output_section* rand = NULL;

    uint64_t cur_size = 0;
    ShuffleInfo::ReorderInfo_LayoutInfo* last_layout = NULL;
    ...
    merge_layout(ShuffleInfo::ReorderInfo& reorder_info,
                 Sized_relobj_file<size, big_endian> *containing_object,
                 const char* insname, uint32_t ins_shndx, uint64_t insize);

    template<int size, bool big_endian> void
    merge_fixup(ShuffleInfo::ReorderInfo &to_add_info,
                Sized_relobj_file<size, big_endian> *pfrom_object,
                Symbol_table *symtab);

    template<int size, bool big_endian> void
    set_fixup(ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple *fixup_output,
    ...
    template<int size, bool big_endian> void
    update_shuffleInfo(Symbol_table* symtab);

    template<int size, bool big_endian>
    void is_need_output_rand();

    ShuffleInfo::ReorderInfo& get_global_shuffleInfo()
    {return this->shuffle_info_.global_reorder_info;}

    bool is_fixup_section(const char *sec_name);

    void yarp_add_fixups(uint64_t fixup_offset, const char *sec_name,
